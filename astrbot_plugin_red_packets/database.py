...
class RedPacketDatabase:
    ...
    async def _init_db(self):
        ...
        async with self.conn.execute("PRAGMA table_info(active_packets)") as cursor:
            ...
            if 'unified_msg_origin' not in columns:
                await self.conn.execute("ALTER TABLE active_packets ADD COLUMN unified_msg_origin TEXT")
            if 'amount_per_packet' not in columns:
                await self.conn.execute("ALTER TABLE active_packets ADD COLUMN amount_per_packet INTEGER")

        # 检查并为 history 表添加新列
        async with self.conn.execute("PRAGMA table_info(packet_history)") as cursor:
            columns = [row[1] for row in await cursor.fetchall()]
            if 'sender_name' not in columns:
                await self.conn.execute("ALTER TABLE packet_history ADD COLUMN sender_name TEXT")
    ...
    async def add_active_packet(self, packet_data: Dict[str, Any]):
        ...
        await self.conn.execute(
            """INSERT INTO active_packets (packet_id, packet_type, sender_id, sender_name, group_id, unified_msg_origin, created_at, expires_at,
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                packet_data['packet_id'], packet_data['packet_type'], packet_data['sender_id'],
                packet_data['sender_name'], packet_data['group_id'], packet_data['unified_msg_origin'], 
                packet_data['created_at'], packet_data['expires_at'], packet_data['total_amount'], 
                packet_data['remaining_packets'], packet_data['greeting'], packet_data.get('password'), 
                json.dumps(packet_data.get('amounts_list')), json.dumps(packet_data.get('claimed_by', {})),
    ...
    async def log_transaction(self, user_id: str, packet_id: str, action_type: str, amount: int, 
                              related_user_id: Optional[str] = None, fee: int = 0, sender_name: Optional[str] = None):
        await self._ensure_connected()
        await self.conn.execute(
            """INSERT INTO packet_history (user_id, packet_id, action_type, amount, related_user_id, fee, sender_name) 
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (user_id, packet_id, action_type, amount, related_user_id, fee, sender_name)
        )
        await self.conn.commit()
            ...
            history['received'] = [
                {
                    ...
                    'sender_name': row['sender_name'] if row['sender_name'] else '历史红包'
                }
                for row in rows
            ]
        return history
    ...
    async def get_active_packet(self, packet_id: str) -> Optional[Dict]:
        await self._ensure_connected()
        async with self.conn.execute("SELECT * FROM active_packets WHERE packet_id = ?", (packet_id,)) as cursor:
            row = await cursor.fetchone()
            return dict(row) if row else None
            
    async def get_packet_by_password(self, password: str) -> Optional[Dict]:
        await self._ensure_connected()
        async with self.conn.execute("SELECT * FROM active_packets WHERE password = ? AND remaining_packets > 0", (password,)) as cursor:
            row = await cursor.fetchone()
            return dict(row) if row else None

    async def get_user_active_packet(self, user_id: str, group_id: str) -> Optional[Dict]:
        await self._ensure_connected()
    ...
    async def get_expired_packets(self) -> List[Dict]:
        await self._ensure_connected()
        now = datetime.now().isoformat()
        async with self.conn.execute("SELECT * FROM active_packets WHERE expires_at <= ?", (now,)) as cursor:
            return [dict(row) for row in await cursor.fetchall()]

    async def update_packet_claim(self, packet_id: str, remaining_packets: int, amounts_json: str, claimed_by_json: str):
        await self._ensure_connected()
        await self.conn.execute(
    ...
    async def remove_active_packet(self, packet_id: str):
        await self._ensure_connected()
        await self.conn.execute("DELETE FROM active_packets WHERE packet_id = ?", (packet_id,))
        await self.conn.commit()