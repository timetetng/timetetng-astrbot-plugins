...
# -- 插件元数据 --
__PLUGIN_METADATA__ = {
    "name": "astrbot_plugin_red_packets",
    "author": "timetetng",
...
class RedPacketPlugin(Star):
    def __init__(self, context: Context):
        ...
        self.expiration_check_task = asyncio.create_task(
            self._check_expired_packets_loop()
        )
                ...
                for packet in expired_packets:
                    ...
                    if refund_amount > 0:
                        ...
                        if success and packet["unified_msg_origin"]:
                            new_balance = await economy_api.get_coins(
                                packet["sender_id"]
                            )
                            ...
                            await self.context.send_message(
                                packet["unified_msg_origin"], msg_chain_obj
                            )
    ...
    def _generate_lucky_amounts(self, total_amount: int, num_packets: int) -> List[int]:
        if num_packets <= 0 or total_amount <= 0:
            return []
        if num_packets == 1:
            return [total_amount]
    ...
    async def _build_summary_message(
        self, packet_dict: Dict[str, Any], final_claimed_by: Dict[str, Any]
    ) -> str:
        lines = [f"🧧「{packet_dict['sender_name']}」的红包已被领完！"]
        lines.append(
            f"总金额 {packet_dict['total_amount']} 金币, 共 {len(final_claimed_by)} 个。"
        )
        lines.append("---领取详情---")
        ...
        if packet_dict["packet_type"] in ["lucky", "password"] and final_claimed_by:
            lucky_king_id = max(
                final_claimed_by, key=lambda uid: final_claimed_by[uid]["amount"]
            )
            lucky_king_data = final_claimed_by[lucky_king_id]
            lines.append("----------------")
            lines.append(
                f"👑 {lucky_king_data['name']} 是手气王，领到了 {lucky_king_data['amount']} 金币！"
            )
            ...
            if "领红包" in msg_text:
                for component in event.message_obj.message:
                    if isinstance(component, Comp.At):
                        group_id = (
                            event.get_group_id() or f"private_{event.get_sender_id()}"
                        )
                        at_user_id = str(component.qq)
                        if packet_dict := await self.db.get_user_active_packet(
                            at_user_id, group_id
                        ):
                            await self._process_claim(event, packet_dict)
                            return
    ...
    async def check_unclaimed_packets(self, event: AstrMessageEvent):
        ...
        for p in packets:
            ...
            claim_method = (
                f"口令: {p['password']}"
                if p["packet_type"] == "password"
                else f"指令: @{p['sender_name']} 领红包"
            )
            response_lines.append(
                f"🧧 来自「{p['sender_name']}」, 剩 {p['remaining_packets']} 个, {minutes}分{seconds}秒后失效。({claim_method})"
            )
    ...
    async def show_records(self, event: AstrMessageEvent):
        ...
        lines = [
            f"🧧 {await self._get_display_name(user_id, event.get_sender_name())} 的红包记录："
        ]
        lines.append(f"📤 总计发出: {len(history['sent'])}个, 共 {sent_total} 金币")
        lines.append(
            f"📥 总计收到: {len(history['received'])}个, 共 {received_total} 金币"
        )
        ...
        if not history["received"]:
            ...
            for r in history["received"][:5]:
                lines.append(
                    f"[{r['timestamp'][:10]}] 收到 {r['sender_name']} 的红包: +{r['amount']}"
                )
        yield event.plain_result("\n".join(lines))
...
▸ `/红包记录` - 查看自己的收发历史。"""
    ...
    async def send_lucky(
        ...
        *,
        ...
        if sender_balance < total_cost:
            yield event.plain_result(
                f"😥 金币不足！需要 {total_cost} (含手续费)，你只有 {int(sender_balance)}。"
            )
            return
        if not await economy_api.add_coins(sender_id, -total_cost, "发送拼手气红包"):
        ...
        packet_data = {
            ...
            "sender_name": await self._get_display_name(
                sender_id, event.get_sender_name()
            ),
            "group_id": group_id,
            ...
            "greeting": greeting,
            "amounts_list": self._generate_lucky_amounts(total, count),
            "claimed_by": {},
    ...
    async def send_fixed(
        ...
        *,
        ...
        if total < count:
            yield event.plain_result(
                f"⚠️ 总金额 ({total}) 不能小于红包个数 ({count})，否则没人能领到钱！"
            )
            return
        ...
        packet_data = {
            ...
            "created_at": now.isoformat(),
            "expires_at": expires.isoformat(),
            "total_amount": distributable_total,
            "remaining_packets": count,
            "greeting": greeting,
            "amount_per_packet": amount_per,
            "claimed_by": {},
            "sender_id": sender_id,
            "unified_msg_origin": event.unified_msg_origin,
        }
    ...
    async def _process_claim(
        self, event: AstrMessageEvent, packet_dict: Dict[str, Any]
    ):
        ...
        if amount <= 0:
            await event.send(
                event.plain_result("💨 这个小红包是空的，下次手速快点哦！")
            )

        economy_api = await self._get_economy_api()
        if amount > 0:
            if not await economy_api.add_coins(
                claimer_id, amount, f"领取{packet_dict['sender_name']}的红包"
            ):
        ...
        await self.db.update_packet_claim(
            ...
            remaining_packets,
            json.dumps(amounts_list),
            json.dumps(claimed_by),
        )
        ...
        if amount > 0:
            await event.send(
                event.plain_result(
                    f"🎉 恭喜 {claimer_name} 领取了 {packet_dict['sender_name']} 的红包，获得 {amount} 金币！\n💰 您的余额: {int(new_balance)} 金币"
                )
            )
