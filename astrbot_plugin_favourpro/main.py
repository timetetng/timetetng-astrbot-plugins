...
class DatabaseManager:
    ...
    async def init_db(self):
        ...
        if "daily_favour_gain" not in columns:
            await self._db.execute(
                "ALTER TABLE user_states ADD COLUMN daily_favour_gain INTEGER DEFAULT 0"
            )
        if "last_update_date" not in columns:
            await self._db.execute(
                "ALTER TABLE user_states ADD COLUMN last_update_date TEXT DEFAULT '1970-01-01'"
            )
        if "daily_gift_gain" not in columns:
            await self._db.execute(
                "ALTER TABLE user_states ADD COLUMN daily_gift_gain INTEGER DEFAULT 0"
            )
        if "relationship_lock_until" not in columns:
            await self._db.execute(
                "ALTER TABLE user_states ADD COLUMN relationship_lock_until INTEGER DEFAULT 0"
            )
    ...
    async def update_user_state(
        self, user_id: str, new_state: Dict[str, Any], session_id: Optional[str] = None
    ):
        ...
        daily_gain = new_state.get(
            "daily_favour_gain", self.DEFAULT_STATE["daily_favour_gain"]
        )
        update_date = new_state.get(
            "last_update_date", self.DEFAULT_STATE["last_update_date"]
        )
        daily_gift_gain = new_state.get(
            "daily_gift_gain", self.DEFAULT_STATE["daily_gift_gain"]
        )
        relationship_lock_until = new_state.get(
            "relationship_lock_until", self.DEFAULT_STATE["relationship_lock_until"]
        )
...
class FavourProAPI:
    ...
    async def add_favour(
        self, user_id: str, amount: int, session_id: Optional[str] = None
    ):
        """为指定用户增加或减少好感度。"""
    ...
    async def set_attitude(
        self, user_id: str, attitude: str, session_id: Optional[str] = None
    ):
        """设置用户在机器人心中的印象。"""
        current_state = await self.db.get_user_state(user_id, session_id)
    ...
    async def set_relationship(
        self, user_id: str, relationship: str, session_id: Optional[str] = None
    ):
        """设置用户与机器人的关系。"""
        current_state = await self.db.get_user_state(user_id, session_id)
...
class FavourProPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.block_pattern = re.compile(
            r"\[\s*(?:Favour:|Attitude:|Relationship:).*?\]", re.DOTALL
        )
        self.favour_pattern = re.compile(r"Favour:\s*(-?\d+)")
        self.attitude_pattern = re.compile(
            r"Attitude:\s*(.+?)(?=\s*,\s*Relationship:|\])"
        )
        self.relationship_pattern = re.compile(r"Relationship:\s*(.+?)(?=\s*\])")
        self.daily_favour_limit = 100
        self.daily_gift_limit = 30
    ...
    async def on_llm_resp(self, event: AstrMessageEvent, resp: LLMResponse):
        ...
        if block_match:
            ...
            if favour_match:
                ...
                current_state = await self.db_manager.get_user_state(
                    user_id, session_id
                )
                ...
                if gain > 5:
                    logger.warning(
                        f"LLM为用户 {user_id} 提出了过高的好感度增益 ({gain})，已强制修正为 +5。"
                    )
                    gain = 5
                elif gain < -10:
                    logger.warning(
                        f"LLM为用户 {user_id} 提出了过高的好感度减损 ({gain})，已强制修正为 -10。"
                    )
                    gain = -10
                ...
                if not is_locked:
                    ...
                    if relationship_match:
                        current_state["relationship"] = relationship_match.group(
                            1
                        ).strip(" ,")
                else:
                    if attitude_match or relationship_match:
                        logger.info(
                            f"用户 {user_id} 的关系和印象处于锁定状态，本次对话引起的变更已被忽略。"
                        )

                # 修正点 3: 将数据库更新操作移到最外层，确保每次都执行
                await self.db_manager.update_user_state(
                    user_id, current_state, session_id
                )
    ...
    async def show_favour_ranking(self, event: AstrMessageEvent):
        ...
        for i, user in enumerate(ranking_data):
            ...
            response_lines.append(
                f"❤️{i + 1}. {display_name}      {favour_score} ({relationship})"
            )
    ...
    async def query_status(self, event: AstrMessageEvent):
        ...
        if lock_until_ts > now_ts:
            lock_end_time = datetime.fromtimestamp(lock_until_ts).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    async def admin_set_favour(self, event: AstrMessageEvent, *, content: str):
        ...
        yield event.plain_result(
            f"成功：用户 {target_id} 的全局好感度已设置为 {favour_value}。"
        )
    ...
    async def admin_set_attitude(self, event: AstrMessageEvent, *, content: str):
        ...
        # 从纯文本参数中解析出印象内容
        attitude_parts = [
            part for part in content.strip().split() if not part.startswith("@")
        ]
        ...
        yield event.plain_result(
            f"成功：用户 {target_id} 的全局印象已设置为 '{attitude}'。"
        )
    ...
    async def admin_set_relationship(self, event: AstrMessageEvent, *, content: str):
        ...
        yield event.plain_result(
            f"成功：用户 {target_id} 的全局关系已设置为 '{relationship}'。"
        )
    ...
    async def gift_to_bot(self, event: AstrMessageEvent):
        ...
        if not shop_api or not eco_api:
            yield event.plain_result("错误：商店或经济系统未启用，无法赠送礼物。")
            return
        ...
        if not item_name:
            ...
            return
        if quantity <= 0:
            yield event.plain_result("赠送数量必须是正数哦~")
            return
        ...
        for item in self.item_manager.items_map.values():
            if item["name"] == item_name:
                ...
                break
        if not item_info:
            yield event.plain_result(f"菲比好像不认识名为“{item_name}”的礼物呢…")
            return
        ...
        item_in_inventory = next(
            (inv_item for inv_item in inventory if inv_item["item_id"] == item_id), None
        )
        ...
        if item_in_inventory and item_in_inventory.get("quantity", 0) >= quantity:
            ...
            if consumed:
                ...
                yield event.plain_result("尝试从背包使用礼物失败，请稍后再试。")
                return
        else:
            # 背包数量不足或没有，进入金币购买流程
            shop_item_details = await shop_api.get_item_details(item_id)
            if not shop_item_details:
                yield event.plain_result("错误：该物品当前未在商店上架。")
                return

            # 每日限购检查
            daily_limit = shop_item_details.get("daily_limit", 0)
            if daily_limit > 0:
                current_purchase_count = await shop_api.get_today_purchase_count(
                    sender_id, item_id
                )
            ...
            if sender_balance < total_price:
                yield event.plain_result(
                    f"赠送失败，你的金币不足！\n需要支付 {total_price} 金币，你只有 {sender_balance} 金币。"
                )
                return
        ...
        if payment_success:
            ...
            bot_state_about_user = await self.db_manager.get_user_state(
                sender_id, session_id=None
            )
            ...
            if effect_type == "add_favour":
                ...
                if (
                    bot_state_about_user.get("daily_gift_gain", 0)
                    >= self.daily_gift_limit
                ):
                    if not consumed_from_inventory:
                        yield event.plain_result(
                            f"你成功购买了“{item_name}”，但菲比今天收到的礼物太多啦！心意领了，不过好感度要明天才能增加了哦~"
                        )
                        return
                    else:
                        yield event.plain_result(
                            f"你使用了“{item_name}”，但菲比今天收到的礼物太多啦！心意领了，不过好感度要明天才能增加了哦~"
                        )
                        return

                gain_value = effect["value"] * quantity
                if (
                    bot_state_about_user.get("daily_gift_gain", 0) + gain_value
                    > self.daily_gift_limit
                ):
                    gain_value = self.daily_gift_limit - bot_state_about_user.get(
                        "daily_gift_gain", 0
                    )

                bot_state_about_user["daily_gift_gain"] = (
                    bot_state_about_user.get("daily_gift_gain", 0) + gain_value
                )
                ...
                if consumed_from_inventory:
                    yield event.plain_result(
                        f"你从背包中拿出 {quantity}份“{item_name}”送给了菲比，她的好感度提升了 {gain_value} 点！"
                    )
                else:
                    new_balance = await eco_api.get_coins(sender_id)
                    yield event.plain_result(
                        f"你赠送了 {quantity}份“{item_name}”，菲比对你的好感度提升了 {gain_value} 点！\n💰消费 {total_price} 金币，剩余 {new_balance} 金币。"
                    )

            elif effect_type == "reset_favour":
                if quantity > 1:
                    yield event.plain_result("好感度重置卡一次只能使用一张哦。")
                    return
                default_state = self.db_manager.DEFAULT_STATE
                await self.api.set_favour(
                    sender_id, default_state["favour"], session_id=None
                )
                await self.api.set_attitude(
                    sender_id, default_state["attitude"], session_id=None
                )
                await self.api.set_relationship(
                    sender_id, default_state["relationship"], session_id=None
                )

                if consumed_from_inventory:
                    yield event.plain_result(
                        f"你从背包中拿出了“{item_name}”，你和菲比之间的一切都回到了原点…"
                    )
                else:
                    new_balance = await eco_api.get_coins(sender_id)
                    yield event.plain_result(
                        f"你使用了“{item_name}”，你和菲比之间的一切都回到了原点…\n💰消费 {total_price} 金币，剩余 {new_balance} 金币。"
                    )
        else:
    ...
    async def use_item(self, event: AstrMessageEvent):
        ...
        if not item_name:
            ...
            return
        if quantity <= 0:
            yield event.plain_result("使用数量必须是正数哦~")
            return
        ...
        eco_api = shared_services.get("economy_api")
        if not shop_api or not eco_api:
            yield event.plain_result("错误：商店或经济系统未启用，无法使用道具。")
            return
        ...
        for item in self.item_manager.items_map.values():
            if item["name"] == item_name:
                ...
                break
        if not item_info:
            yield event.plain_result(f"菲比好像不认识名为“{item_name}”的道具呢…")
            return
        ...
        if effect_type not in ["lock_relationship", "reset_favour"]:
            yield event.plain_result(
                f"“{item_name}”好像不能在这里使用呢，也许要通过其他方式？"
            )
            return
        ...
        if item_in_inventory and item_in_inventory.get("quantity", 0) >= quantity:
            ...
            if consumed:
                ...
                yield event.plain_result("尝试从背包使用道具失败，请稍后再试。")
                return
        else:
            shop_item_details = await shop_api.get_item_details(item_id)
            if not shop_item_details:
        ...
        if payment_success:
            if effect_type == "lock_relationship":
                ...
                await self.db_manager.update_user_state(
                    sender_id, current_state, session_id=None
                )

                lock_end_time = datetime.fromtimestamp(new_expiry_ts).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )

                if consumed_from_inventory:
                    yield event.plain_result(
                        f"✨ 你从背包中使用了 {quantity} 张“{item_name}”！\n你与菲比的关系已锁定至 {lock_end_time}。"
                    )
                else:
                    new_balance = await eco_api.get_coins(sender_id)
                    yield event.plain_result(
                        f"✨ 成功购买并使用了 {quantity} 张“{item_name}”！\n你与菲比的关系已锁定至 {lock_end_time}。\n💰消费 {total_price} 金币，剩余 {new_balance} 金币。"
                    )
    ...
    async def unlock_relationship(self, event: AstrMessageEvent):
        ...
        if lock_until_ts > now_ts:
            ...
            await self.db_manager.update_user_state(
                sender_id, current_state, session_id
            )
            yield event.plain_result(
