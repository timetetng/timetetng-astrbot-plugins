...
class LLMBankerPlugin(Star):
    ...
    async def initialize_plugin(self):
        ...
        if self.economy_api and self.stock_api:
            ...
            self._tax_task_handle = asyncio.create_task(self._daily_tax_collection_task())
        else:
    ...
    async def _daily_reset_task(self):
        ...
        try: # [修改] 添加异常捕获以响应取消
            while True:
                ...
                logger.info(f"LLM Banker: 将在 {seconds_until_midnight:.0f} 秒后重置每日津贴。")
        ...
        try: # [修改] 添加异常捕获以响应取消
            while True:
                ...
                try:
                    ...
                    logger.error(f"配置的收税时间 '{tax_time_str}' 格式无效，将使用默认值 00:00:05。")
                ...
                logger.info(f"LLM Banker: 将在 {seconds_until_tax_time:.0f} 秒后 (即 {next_tax_time}) 开始征收资产税。")
                ...
                if not isinstance(tax_rates, list) or len(tax_rates) != 10:
                    logger.warning(f"配置中的 'tax_rates' 项缺失或格式不正确（应为10个浮点数的列表），跳过本次收税。当前值: {tax_rates}")
                    continue 
                ...
                try:
                    ...
                    for i, player_data in enumerate(ranking):
                        ...
                        if isinstance(assets_data, (list, tuple)) and assets_data:
                            try:
                                total_assets_numeric = float(assets_data[0])
                                 ...
                                 logger.warning(f"玩家 {user_id} 的资产数据是序列，但第一个元素不是有效数字或为空: {assets_data}")
                                 total_assets_numeric = 0.0
                        elif isinstance(assets_data, (int, float)):
                            total_assets_numeric = float(assets_data)
                        else:
                            logger.warning(f"玩家 {user_id} 的资产数据格式无法识别 (非数字/序列)，将计为0. 数据: {assets_data} (类型: {type(assets_data)})")
                            total_assets_numeric = 0.0
                        ...
                        try:
                            ...
                            logger.error(f"玩家 {user_id} 的税率 tax_rates[{i}] (值: '{tax_rates[i]}') 无法转换为浮点数，跳过此玩家。")
                            continue # 跳过这个玩家
                        # --- [修复 2 结束] ---
                        ...
                        logger.info(f"向排行第 {i+1} 的玩家 {user_id} (总资产: {assets_data}) 征收 {tax_rate*100:.2f}% 的税，金额: {tax_amount}")

                        op1 = await self.economy_api.add_coins(user_id, -tax_amount, "每日资产税")
                        op2 = await self.economy_api.add_coins(bot_id, tax_amount, f"收取来自 {user_id} 的每日资产税")
                        
                        if not (op1 and op2):
                            logger.error(f"向玩家 {user_id} 收税时发生错误，尝试回滚。")
                            await self.economy_api.add_coins(user_id, tax_amount, "收税失败回滚")
                            await self.economy_api.add_coins(bot_id, -tax_amount, "收税失败回滚")
    ...
    async def evaluate_user_financial_status(self, event: AstrMessageEvent, target_user_id: str) -> str:
        """
    ...
    async def transfer_coins_to_user(self, event: AstrMessageEvent, target_user_id: str, amount: int, reason: str) -> Dict[str, Any]:
        """
        ...
        if current_allowance + amount > daily_limit:
            return {"success": False, "error": f"该用户今日已达到获取上限，无法再向其转账。"}

        bot_balance = await self.economy_api.get_coins(bot_id)
        low_balance_threshold = self.config.get("low_balance_threshold", 20000)
        if bot_balance < low_balance_threshold:
            return {"success": False, "error": f"我的余额已低于警戒线({low_balance_threshold})，现在不能乱花钱。"}
        if bot_balance < amount:
            return {"success": False, "error": f"我的钱不够了，当前只有 {bot_balance} 金币。"}

        op1 = await self.economy_api.add_coins(bot_id, -amount, f"转账给 {target_user_id} ({reason})")
        op2 = await self.economy_api.add_coins(target_user_id, amount, f"收到机器人转账 ({reason})")

        if op1 and op2:
            daily_allowance_tracker[target_user_id] = current_allowance + amount
            return {"success": True, "message": f"操作成功，已向用户 {target_user_id} 转账 {amount} 金币。"}
        else:
            await self.economy_api.add_coins(bot_id, amount, f"交易失败回滚")
            await self.economy_api.add_coins(target_user_id, -amount, f"交易失败回滚")
            return {"success": False, "error": "转账过程中发生未知错误，交易已取消。"}


    @filter.llm_tool(name="take_coins_from_user")
    async def take_coins_from_user(self, event: AstrMessageEvent, target_user_id: str, amount: int, reason: str) -> Dict[str, Any]:
        """
        ...
        op1 = await self.economy_api.add_coins(target_user_id, -amount, f"被机器人扣款 ({reason})")
        op2 = await self.economy_api.add_coins(bot_id, amount, f"从 {target_user_id} 处收款 ({reason})")

        if op1 and op2:
            return {"success": True, "message": f"操作成功，已从用户 {target_user_id} 处取走 {amount} 金币。"}
        else:
            await self.economy_api.add_coins(target_user_id, amount, f"交易失败回滚")
            await self.economy_api.add_coins(bot_id, -amount, f"交易失败回滚")
