...
try:
    ...
    logger.error("RedeemCode 插件缺少 fastapi 或 uvicorn 依赖。请创建 requirements.txt 并重启 AstrBot。")
...
class RedeemCodePlugin(Star):
    def __init__(self, context: Context, config: dict):
        ...
        data_root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    ...
    def _load_data(self):
        try:
            self.codes = json.load(open(self.codes_file, 'r', encoding='utf-8')) if os.path.exists(self.codes_file) else {}
            self.usage_records = json.load(open(self.usage_file, 'r', encoding='utf-8')) if os.path.exists(self.usage_file) else {"universal_codes": {}, "single_use_codes": {}}
    ...
    def _generate_code_data(self, code_type: str, amount: int, duration: str) -> Optional[Dict[str, Any]]:
        time_delta = self._parse_duration(duration)
        if not time_delta: return None
        
        while True:
            new_code = uuid.uuid4().hex[:12].upper()
            if new_code not in self.codes: break
    ...
    async def terminate(self):
        if self.uvicorn_server and self.uvicorn_server.started:
            ...
            # uvicorn.Server.shutdown() is not a coroutine in all versions
            # It's safer to call it directly and let the event loop handle it.
    ...
    def setup_api_routes(self):
        ...
        async def api_create_code(request: CreateCodeRequest, api_key: str = Depends(get_api_key)):
            code_data = self._generate_code_data(request.code_type, request.amount, request.duration)
            if not code_data:
            ...
            return CreateCodeResponse(
                ...
                expires_at_str=datetime.fromtimestamp(code_data["expires_at"]).strftime('%Y-%m-%d %H:%M:%S'),
    ...
    async def create_code(self, event: AstrMessageEvent, code_type: str, amount: int, duration: str):
        if code_type not in ["universal", "single"]:
            yield event.plain_result("错误：类型必须是 'universal' (通用码) 或 'single' (一次性码)。")
            return
        if amount <= 0:
        ...
        if not code_data:
            yield event.plain_result("错误：无效的有效期格式。请使用如 '7d', '24h', '30m' 的格式。")
            return
        ...
        expire_time_str = datetime.fromtimestamp(code_data["expires_at"]).strftime('%Y-%m-%d %H:%M:%S')
        type_str = "通用码 (每人可用一次)" if code_data["type"] == "universal" else "一次性码 (仅限一人使用)"
        reply_msg = (f"✅ 兑换码创建成功！\n"
                     f"码: {code}\n"
                     f"类型: {type_str}\n"
                     f"奖励: {amount} 金币\n"
    ...
    async def redeem_code(self, event: AstrMessageEvent, code: str):
        ...
        if code_type == "single":
            ...
            if code in self.usage_records.get("universal_codes", {}) and user_id in self.usage_records["universal_codes"].get(code, []):
                yield event.plain_result("❌ 您已经兑换过此奖励。")
                return
        
        reward_amount = code_data["reward_amount"]
        try:
            success = await self.economy_api.add_coins(user_id=user_id, amount=reward_amount, reason=f"兑换码: {code}")
            if not success: raise Exception("EconomyAPI add_coins returned False")
        except Exception as e:
        ...
        if code_type == "single":
            self.usage_records.setdefault("single_use_codes", {})[code] = {"user_id": user_id, "timestamp": time.time()}
        elif code_type == "universal":
            self.usage_records.setdefault("universal_codes", {}).setdefault(code, []).append(user_id)
        self._save_usage()
