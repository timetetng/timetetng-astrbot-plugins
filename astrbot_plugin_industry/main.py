...
class IndustryPlugin(Star):
    ...
    async def get_asset_value_for_api(self, user_id: str) -> int:
        ...
        if company.get('is_public'):
            ...
            if stock_api and hasattr(stock_api, 'get_market_cap'): # 确保API和方法都存在
                try:
                    market_cap = await stock_api.get_market_cap(company['stock_ticker'])
                    if market_cap is not None:
                        return int(market_cap)
                except Exception as e:
                    logger.error(f"调用 stock_api.get_market_cap 时发生错误: {e}", exc_info=True)
    ...
    async def _get_top_companies_for_api(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        tasks = [self.get_asset_value_for_api(company['user_id']) for company in all_companies]
        ...
        # 4. 按资产价值从高到低排序
        sorted_companies = sorted(company_data_with_value, key=lambda x: x['asset_value'], reverse=True)
    ...
    async def create_company_handler(self, event: AstrMessageEvent, company_name: str = ""):
    ...
    async def rename_company_handler(self, event: AstrMessageEvent, new_name: str = ""):
        ...
        if not new_name:
            yield event.plain_result(f"指令格式不正确哦，请使用：/公司改名 [新的公司名]\n改名将消耗金币。")
            return
        user_id = event.get_sender_id()
    ...
    # --- 删除：/选择部门 指令已废弃 ---
    # `choose_department_handler` 函数已被移除

    @filter.command("我的部门", alias={'部门', '查看部门'})
    async def get_department_profile_handler(self, event: AstrMessageEvent):
        """查看你公司所有部门的详细信息和总加成。"""
        result = await self.service.get_department_profile(event.get_sender_id(), event.get_sender_name())
        yield event.plain_result(result)

    # --- 重大修改：/升级部门 指令 ---
    @filter.command("升级部门", alias={'部门升级'})
    async def upgrade_department_handler(self, event: AstrMessageEvent, dept_name: str = ""):
        """提升你指定部门的等级。"""
        ...
        if not final_dept_name:
            yield event.plain_result("请指定要升级的部门：\n- `/升级部门 运营部`\n- `/升级部门 研发部`\n- `/升级部门 公关部`\n(您也可以使用自定义的别名)")
            return
        result = await self.service.upgrade_department(event.get_sender_id(), final_dept_name)
        yield event.plain_result(result)

    # +++ 新增：部门改名指令 +++
    @filter.command("部门改名")
    async def set_department_alias_handler(self, event: AstrMessageEvent, old_name: str = "", new_alias: str = ""):
        """为你的部门设置一个自定义的别名。"""
        if not old_name or not new_alias:
            yield event.plain_result("指令格式不正确哦，请使用：\n`/部门改名 [原名或别名] [新别名]`")
            return
        
        result = await self.service.set_department_alias(event.get_sender_id(), old_name, new_alias)
        yield event.plain_result(result)
     # --- 玩家互动指令 (V3 - 已修改) ---
    ...
    async def talent_poach_handler(self, event: AstrMessageEvent):
        ...
        if not target_id:
            yield event.plain_result("请 @ 一位玩家或提供其ID。例如：\n/挖角 @张三\n/挖角 12345678")
            return
    ...
    async def industrial_espionage_handler(self, event: AstrMessageEvent):
        ...
        result = await self.service.industrial_espionage(event.get_sender_id(), target_id)
        yield event.plain_result(result)
    ...
    @filter.command("公司行动")
    async def corporate_action_handler(self, event: AstrMessageEvent, *, action_name: str = ""):
        ...
        if not action_name:
            ...
            actions_list = [f"- {act['name']} (`/公司行动 {act['name']}`)" for key, act in config.CORPORATE_ACTIONS.items()]
        ...
        result_msg = await self.service.perform_corporate_action(user_id, action_keyword)
        yield event.plain_result(result_msg)
