...
def extract_text_and_images_from_chain(chain: List[object]) -> Tuple[str, List[str]]:
    ...
    for seg in chain:
        try:
            ...
            # 处理合并转发节点 (Node/Nodes/Forward)
            elif hasattr(Comp, "Node") and isinstance(
                seg, getattr(Comp, "Node")
            ):  # Node
                t, i = extract_text_and_images_from_chain(
                    getattr(seg, "content", []) or []
                )
                if t:
                    texts.append(t)
                images.extend(i)
            elif hasattr(Comp, "Nodes") and isinstance(
                seg, getattr(Comp, "Nodes")
            ):  # Nodes
                ...
                for node in nodes:
                    ...
                    if t:
                        texts.append(t)
                    images.extend(i)
            elif hasattr(Comp, "Forward") and isinstance(
                seg, getattr(Comp, "Forward")
            ):  # Forward
                nodes = getattr(seg, "nodes", []) or getattr(seg, "content", []) or []
                for node in nodes:
                    t, i = extract_text_and_images_from_chain(
...
async def extract_quoted_payload(event: AstrMessageEvent) -> List[str]:
    ...
    # 获取 reply_id
    reply_id = getattr(reply_comp, "id", None) or getattr(
        reply_comp, "message_id", None
    )
            ...
            if isinstance(msg_list, list):
                for seg in msg_list:
                    if isinstance(seg, dict) and seg.get("type") == "forward":
                        ...
                        if fid:
                            fwd_ret = await event.bot.api.call_action(
