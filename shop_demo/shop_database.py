...
class ShopDatabase:
    ...
    async def _check_and_add_columns(self):
        ...
        if "daily_limit" not in existing_columns:
            await self.conn.execute("ALTER TABLE items ADD COLUMN daily_limit INTEGER NOT NULL DEFAULT 0")
            logger.info("INFO: shop_plugin > 已成功为 items 表添加 'daily_limit' 字段。")
    ...
    async def add_or_update_item_definition(self, item_id: str, name: str, description: str, price: int, owner_plugin: str, daily_limit: int = 0):
    ...
    async def remove_item_definition(self, item_id: str) -> str:
        ...
        async with self._lock: # 使用锁确保检查和删除操作的原子性
            # 1. 检查是否仍有用户持有该物品
            async with self.conn.execute("SELECT 1 FROM user_inventory WHERE item_id = ? LIMIT 1", (item_id,)) as cursor:
                is_owned = await cursor.fetchone()
                if is_owned:
                    logger.warning(f"无法下架物品 {item_id}，因为它仍存在于用户背包中。")
                    return 'in_use'

            # 2. 如果无人持有，则执行删除
            cursor = await self.conn.execute("DELETE FROM items WHERE item_id = ?", (item_id,))
            await self.conn.commit()
    ...
    async def get_item_by_id(self, item_id: str) -> Optional[Dict]:
        await self._ensure_connected()
        async with self.conn.execute("SELECT * FROM items WHERE item_id = ?", (item_id,)) as cursor:
            row = await cursor.fetchone()
            return dict(row) if row else None

    async def get_item_by_name(self, name: str) -> Optional[Dict]:
        await self._ensure_connected()
        async with self.conn.execute("SELECT * FROM items WHERE name = ?", (name,)) as cursor:
            row = await cursor.fetchone()
            return dict(row) if row else None
    ...
    async def remove_item_from_user(self, user_id: str, item_id: str, quantity: int = 1) -> bool:
        """从用户库存中移除指定数量的物品，如果数量足够则返回True。"""
        await self._ensure_connected()
        # 使用事务确保操作的原子性
        async with self.conn.execute("BEGIN"):
            async with self.conn.execute("SELECT quantity FROM user_inventory WHERE user_id = ? AND item_id = ?", (user_id, item_id)) as cursor:
                result = await cursor.fetchone()
