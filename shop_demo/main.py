...
class ShopAPI:
    ...
    async def register_item(self, owner_plugin: str, item_id: str, name: str, description: str, price: int, daily_limit: int = 0):
        await self._db.add_or_update_item_definition(item_id, name, description, price, owner_plugin, daily_limit)
...
class ShopPlugin(Star):
    ...
    @filter.command("购买")
    async def buy_item(self, event: AstrMessageEvent, identifier: str, quantity: int = 1):
        if quantity <= 0:
        ...
        if not item_to_buy:
            yield event.plain_result(f"抱歉，没有找到编号或名称为“{identifier}”的商品。")
            return
        
        # vvvvv 核心逻辑：每日限购检查 vvvvv
        daily_limit = item_to_buy.get('daily_limit', 0)
        if daily_limit > 0:
            current_purchase_count = await self.db.get_today_purchase_count(user_id, item_to_buy['item_id'])
    ...
    async def gift_item(self, event: AstrMessageEvent, content: str): # content 参数将不再被直接使用
        """
        处理为他人付款购买物品的指令。
        ...
        # 1. 提取所有纯文本部分并拼接
        plain_text = "".join(str(c.text) for c in event.message_obj.message if isinstance(c, Comp.Plain)).strip()
        ...
        if item_name_str:
            ...
            if len(numbers) == 1:
                ...
                yield event.plain_result("指令格式不正确。当只使用数字时，请按 `商品编号 数量` 或 `商品编号` 的格式提供。")
                return
    ...
    async def add_item_interactive(self, event: AstrMessageEvent):
        ...
        try:
            # 启动会话
            yield event.plain_result("好的，我们开始上架新商品。\n第一步，请输入商品的【英文ID】(例如 a_cool_item)，输入 `退出` 可随时取消。")

            @session_waiter(timeout=120)  # 2分钟无操作则自动超时
            async def item_creation_waiter(controller: SessionController, event: AstrMessageEvent):
                ...
                if "item_id" not in item_data:
                    ...
                    if ' ' in user_input: # 简单验证
                        await event.send(event.plain_result("❌ 英文ID不能包含空格，请重新输入。"))
                        return # 继续等待，不重置超时
                    item_data["item_id"] = user_input
                    await event.send(event.plain_result(f"ID已设为: {user_input}\n第二步，请输入商品的【中文名称】。"))

                elif "name" not in item_data:
                    # 步骤2：接收中文名称
                    item_data["name"] = user_input
                    await event.send(event.plain_result(f"名称已设为: {user_input}\n第三步，请输入商品的【价格】(纯数字)。"))
                    ...
                    try:
                        ...
                        await event.send(event.plain_result(f"价格已设为: {price}\n第四步，请输入【每日限购次数】(输入 0 代表不限购)。"))
                    except ValueError:
                        await event.send(event.plain_result("❌ 价格必须是一个非负整数，请重新输入。"))
                    ...
                    try:
                        ...
                        await event.send(event.plain_result(f"每日限购已设为: {limit}\n最后一步，请输入商品的【功能描述】。"))
                    except ValueError:
                        await event.send(event.plain_result("❌ 限购次数必须是一个非负整数，请重新输入。"))
                    ...
                    if user_input == "确认":
                        ...
                        await event.send(event.plain_result(f"✅ 操作成功！商品【{item_data['name']}】已成功上架/更新。"))
                    else:
                        await event.send(event.plain_result("操作已取消。"))
    ...
    async def show_item_info(self, event: AstrMessageEvent, identifier: str):
        ...
        if not item_details:
            yield event.plain_result(f"❌ 未在商店中找到编号或名称为“{identifier}”的物品。")
            return

        # 格式化输出
        limit_text = '不限购' if item_details['daily_limit'] == 0 else str(item_details['daily_limit'])
        reply = (
            f"---------- 物品详情 ----------\n"
