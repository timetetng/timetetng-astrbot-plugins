...
# [新增] 尝试导入经济和昵称系统API
# 注意：这需要你的经济、昵称插件提供一个可供导入的 `shared_services` 对象。
# 通常这要求插件间遵循一定的目录结构，例如将公共服务放在一个共享的 `common` 目录中。
try:
    from ..common.services import shared_services
except (ImportError, ModuleNotFoundError):
    logger.warning("无法导入 'shared_services'。经济和昵称系统功能将不可用。请检查插件结构。")
...
class WordleGame:
    ...
    async def gen_image(self) -> bytes:
        CELL_COLORS = {
            2: (106, 170, 100), 1: (201, 180, 88), 0: (120, 124, 126), -1: (211, 214, 218),
        }
        ...
        for row in range(self.max_attempts):
            ...
            for col in range(self.length):
                ...
                if row < len(self.guesses) and col < len(self.guesses[row]):
                    letter, feedback_value = self.guesses[row][col].upper(), self.feedbacks[row][col]
                    cell_color = CELL_COLORS[feedback_value]
                else:
                    letter, cell_color = "", CELL_COLORS[-1]
                draw.rectangle([x, y, x + CELL_SIZE, y + CELL_SIZE], fill=cell_color, outline=None)
                if letter:
                    text_bbox = draw.textbbox((0, 0), letter, font=self._font)
                    text_width, text_height = text_bbox[2] - text_bbox[0], text_bbox[3] - text_bbox[1]
                    letter_x, letter_y = x + (CELL_SIZE - text_width) // 2 + 2.5, y + (CELL_SIZE - text_height) // 2 + 1
        ...
        for i in range(self.length):
            if feedback[i] != 2:
                ...
                if char in answer_char_counts and answer_char_counts[char] > 0:
                    feedback[i] = 1; answer_char_counts[char] -= 1
        self.feedbacks.append(feedback)
        newly_correct = 0
        for i, result in enumerate(feedback):
            if result == 2 and i not in self.correct_positions:
                newly_correct += 1; self.correct_positions.add(i)
        if newly_correct > 0:
            self.player_contributions[user_id] = self.player_contributions.get(user_id, 0) + newly_correct
            logger.info(f"玩家 {user_id} 贡献了 {newly_correct} 个新线索。总贡献: {self.player_contributions}")
        return await self.gen_image()

    async def hint(self) -> bytes:
        if not any(char in self.history_letters for char in self.answer):
            logger.info("用户还未猜出任何字母。"); return False
        hint_word = ""; tem1 = self.history_letters.copy()
        for char in self.answer:
            if char in tem1:
                hint_word += char; tem1.remove(char)
            else:
                hint_word += " "
        return await self.gen_image_hint(hint_word.upper())

    @property
    def is_game_over(self): return self.guesses and len(self.guesses) >= self.max_attempts
    @property
    def is_won(self): return self.guesses and self.guesses[-1].upper() == self.answer


register(
    "astrbot_plugin_wordle_2_msg", "Raven95676, whzc, Gemini", "Wordle游戏（响应消息内容版），集成了经济和昵称系统", "3.3.0",
...
class PluginWordle(Star):
    ...
    # --- [新增] 加载 all_wordlist 中的所有单词用于验证 ---
    def _load_validation_words(self) -> Set[str]:
        """从 all_wordlist 文件夹加载所有单词到一个集合中用于快速验证。"""
        validation_set = set()
        try:
            wordlist_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "all_wordlist")
            ...
            for word_file in os.listdir(wordlist_path):
                ...
                with open(os.path.join(wordlist_path, word_file), "r", encoding="utf-8") as f:
                    try:
                        ...
                        validation_set.update([word.upper() for word in word_dict.keys()])
    ...
    async def _async_init(self):
        ...
        if self.economy_api:
            ...
            logger.error("Wordle插件：金币奖励已启用，但未能获取经济系统API！奖励功能将无法使用。")
    ...
    @property
    def custom_word_list(self): return self.config.get("custom_word_list", "").split(";")

    @staticmethod
    async def get_answer(length):
        try:
            wordlist_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wordlist")
            if not os.path.exists(wordlist_path):
                logger.error("词表文件夹不存在")
                return None, None

            # [修正] 创建一个列表来存储所有符合条件的单词和释义，而不是使用会覆盖的字典
            eligible_words = []
    ...
    # [新增] 统一的奖励发放函数，处理每日上限
    async def _award_coins(self, user_id: str, potential_amount: int, reason: str) -> (int, str):
        """
        处理金币奖励，包括每日上限检查。
        返回 (实际奖励数额, 附加消息)
        """
        ...
        if daily_limit <= 0:  # 0或负数表示无限制
            success = await self.economy_api.add_coins(user_id=user_id, amount=potential_amount, reason=reason)
            if not success:
                return 0, "（但金币发放失败了...）"
        ...
        # 安全地初始化用户统计数据
        user_stats = self.stats.setdefault(user_id, {"win_count": 0, "dividend_count": 0, "daily_earnings": {}})
        ...
        success = await self.economy_api.add_coins(user_id=user_id, amount=actual_amount, reason=reason)
        
        if success:
            daily_earnings_dict[today_str] = current_earnings + actual_amount
            message = f"（已达到今日 {daily_limit} 金币上限）" if actual_amount < potential_amount else ""
            return actual_amount, message
        else:
            return 0, "（但金币发放失败了...）"


    async def _handle_win(self, event: AstrMessageEvent, game: WordleGame, explanation: str):
        ...
        if self.achievement_api and len(game.guesses) == 1 and game.length >= 5:
            ...
            was_unlocked = await self.achievement_api.unlock_achievement(
                user_id=winner_id,
                achievement_id="wordle_first_try_win",
                event=event
            )
            if was_unlocked:
                logger.info(f"用户 {winner_id} 通过猜单词一击制胜解锁了成就 [wordle_first_try_win]。")

        # --- 更新胜利者统计数据 ---
        self.stats.setdefault(winner_id, {"win_count": 0, "dividend_count": 0, "daily_earnings": {}})
        ...
        penalty_rate = game.hint_used_count * self.config.get("hint_penalty_percentage", 0.2)
        potential_winner_reward = max(0, int(pre_penalty_reward * (1 - min(penalty_rate, 0.9))))
        
        reward_messages = []
        
        # --- 胜利者奖励发放 ---
        if self.config.get("reward_enabled", False):
            awarded_amount, limit_msg = await self._award_coins(winner_id, potential_winner_reward, "Wordle 游戏胜利")
            if awarded_amount > 0:
                reward_messages.append(f"恭喜你猜对了！获得 {awarded_amount} 金币！{limit_msg}")
            else:
        ...
        # --- 分红逻辑 ---
        if self.config.get("clue_dividend_enabled", True) and game.player_contributions and self.economy_api:
            top_contributor_id = max(game.player_contributions, key=game.player_contributions.get)
            top_contribution = game.player_contributions[top_contributor_id]

            if top_contributor_id != winner_id and top_contribution > 0:
                potential_dividend = int(potential_winner_reward * self.config.get("clue_dividend_percentage", 0.15))
                if potential_dividend > 0:
                    awarded_dividend, dividend_limit_msg = await self._award_coins(top_contributor_id, potential_dividend, "Wordle 最佳线索分红")
                    
                    if awarded_dividend > 0:
                        self.stats.setdefault(top_contributor_id, {"win_count": 0, "dividend_count": 0, "daily_earnings": {}})
                        self.stats[top_contributor_id]["dividend_count"] += 1
                        
                        # --- 获取贡献者昵称逻辑 ---
                        display_name = top_contributor_id
                        if self.nickname_api:
                            custom_name = await self.nickname_api.get_nickname(top_contributor_id)
                            if custom_name:
                                display_name = custom_name
                        
                        if display_name == top_contributor_id and self.economy_api:
                            profile = await self.economy_api.get_user_profile(top_contributor_id)
                            if profile and profile.get("nickname"):
                                display_name = profile["nickname"]

                        if display_name == top_contributor_id and event.get_group_id() and event.get_platform_name() == "aiocqhttp":
                            try:
                                ...
                                logger.warning(f"Wordle: 调用平台API获取用户({top_contributor_id})昵称失败: {e}")
                        
                        reward_messages.append(f"\n特别感谢玩家【{display_name}】提供的关键线索，获得 {awarded_dividend} 金币分红！{dividend_limit_msg}")
    ...
    async def on_message(self, event: AstrMessageEvent):
        msg, session_id = event.get_message_str().strip().lower(), event.unified_msg_origin
        if msg in ["猜单词结束", "结束猜单词", "退出猜单词", "猜单词退出"]:
            if session_id in self.game_sessions:
                game, _ = self.game_sessions[session_id]; del self.game_sessions[session_id]
                yield event.plain_result(f"猜单词已结束，正确答案是 {game.answer}。")
            else: yield event.plain_result("游戏还没开始呢！")
            return
        if msg in ["猜单词提示", "提示猜单词"]:
            if session_id in self.game_sessions:
                game, _ = self.game_sessions[session_id]; game.hint_used_count += 1
                image_result_hint = await game.hint()
                if image_result_hint:
                    filename = f"{session_id.replace(':', '')}_hint_{game.hint_used_count}.png"
                    temp_img_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), filename)
                    try:
                        with open(temp_img_path, "wb") as f: f.write(image_result_hint)
                        chain = [Image.fromFileSystem(temp_img_path), Plain("这是你已经猜出的字母。")]
                        yield event.chain_result(chain)
                    finally:
                        if os.path.exists(temp_img_path): os.remove(temp_img_path)
                else:
                    i = random.randint(0, len(game.answer) - 1)
                    yield event.plain_result(f"提示：第 {i + 1} 个字母是 {game.answer[i]}。")
            else: yield event.plain_result("游戏还没开始，输入“猜单词”来开始游戏吧！")
            return
        if msg.startswith("猜单词") or msg.startswith("/猜单词"):
            parts = msg.replace("/猜单词", "猜单词").split(); length_str = parts[1] if len(parts) > 1 else "5"
            try:
                length = int(length_str)
                if not 3 <= length <= 10: yield event.plain_result("单词长度必须在3到10之间哦！"); return
            except ValueError: yield event.plain_result("请输入有效的单词长度数字！"); return
            if session_id in self.game_sessions: del self.game_sessions[session_id]
            answer, explanation = await self.get_answer(length)
            if not answer: yield event.plain_result(random.choice([f"{length}个字母的单词我找不到...", f"{length}个字母的单词太稀有啦！"]))
            else:
        ...
        if session_id in self.game_sessions:
            ...
            if not (msg.isascii() and msg.isalpha()): return
            if len(msg) != game.length:
                yield event.plain_result(f"不太对哦，要输入{game.length}个字母的英语单词🔡。\n输入“猜单词结束”可结束游戏。"); return
            
            # --- [新增] 单词有效性检查 ---
            if not await self.is_valid_word(msg):
                yield event.plain_result("这好像不是一个有效的英文单词哦，换一个试试吧！🤔"); return
            
            if await game.is_guessed(msg): yield event.plain_result("这个单词已经猜过了！"); return
            ...
            if game.is_won:
                ...
                else: game_status = f"恭喜你猜对了！“{game.answer}”的意思是“{explanation}”。"
                del self.game_sessions[session_id]
            elif game.is_game_over:
                game_status = f"机会用完啦！正确答案是“{game.answer}”，意思是“{explanation}”。"
                del self.game_sessions[session_id]
            else: game_status = f"已猜测 {len(game.guesses)}/{game.max_attempts} 次。"
            filename = f"{session_id.replace(':', '')}_game_{len(game.guesses)}.png"
            temp_img_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), filename)
            try:
                with open(temp_img_path, "wb") as f: f.write(image_result)
