...
class SignDatabase:
        ...
        for col, col_type in columns_to_add.items():
            if col not in existing_columns:
                await self.conn.execute(
                    f"ALTER TABLE sign_data ADD COLUMN {col} {col_type}"
                )
    ...
    async def _check_and_add_columns_for_lottery(self):
        ...
        if "fortune_at_time" not in existing_columns:
            await self.conn.execute(
                "ALTER TABLE lottery_history ADD COLUMN fortune_at_time TEXT DEFAULT ''"
            )
            logger.info(
                "数据库升级：为 'lottery_history' 表添加了 'fortune_at_time' 列。"
            )
    ...
    async def get_user_data(self, user_id: str) -> dict[str, Any] | None:
        await self._ensure_connected()
        async with self.conn.execute(
            "SELECT * FROM sign_data WHERE user_id = ?", (user_id,)
        ) as cursor:
    ...
    async def update_user_data(self, user_id: str, **kwargs):
        ...
        if not await self.get_user_data(user_id):
            await self.conn.execute(
                "INSERT OR IGNORE INTO sign_data (user_id) VALUES (?)", (user_id,)
            )
    ...
    async def get_jackpot_wins(self, limit: int = 5) -> list[aiosqlite.Row]:
        ...
        await self._ensure_connected()
        async with self.conn.execute(
            "SELECT nickname, amount, timestamp FROM jackpot_wins ORDER BY timestamp DESC LIMIT ?",
            (limit,),
        ) as cursor:
            return await cursor.fetchall()
    ...
    async def get_total_coin_supply(self) -> int:
        ...
        await self._ensure_connected()
        async with self.conn.execute(
            "SELECT SUM(coins) as total FROM sign_data"
        ) as cursor:
            result = await cursor.fetchone()
    ...
    async def get_net_change_between(
    ...
    async def get_personal_flow_summary(self, user_id: str, days: int = 7) -> dict:
        ...
        if days == 1:
            ...
            time_clause = (
                "DATETIME(timestamp, 'localtime') >= DATETIME('now', ?, 'localtime')"
            )
        ...
        async with self.conn.execute(
            expenditure_query, (user_id, *days_param)
        ) as cursor:
            result = await cursor.fetchone()
            summary["expenditure"] = (
                abs(result["total"]) if result and result["total"] else 0
            )

        return summary

    async def get_personal_lottery_history(
    ...
    async def get_personal_fortune_summary(
        self, user_id: str, days: int = 7
    ) -> str | None:
    ...
    async def get_lottery_luck_ranking(
        self, limit: int = 10, order: str = "DESC"
    ) -> list[aiosqlite.Row]:
        """获取抽奖运气排行榜"""
        await self._ensure_connected()
        query = f"""
    ...
    async def process_luck_change_card_usage(
        self,
        user_id: str,
        new_coins: int,
        cost: int,
    ...
    async def process_lottery_ticket_usage(
        self, user_id: str, cost: int, current_extra_attempts: int
    ) -> int:
        ...
        async with self._lock:  # <-- 也在这里添加异步锁
            try:
                ...
                cursor = await self.conn.execute(
                    "SELECT coins FROM sign_data WHERE user_id = ?", (user_id,)
                )
                row = await cursor.fetchone()
