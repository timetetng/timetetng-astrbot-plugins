...
def clean_and_highlight(desc: str) -> str:
    ...
    cleaned_desc = re.sub(
        r"(【.*?效应】)",
        f'<strong style="color: {effect_color};">\\1</strong>',
        cleaned_desc,
    )
    return cleaned_desc
...
async def process_monsters(http_client: httpx.AsyncClient, monsters_obj: dict) -> list:
    ...
    unique_urls = {
        ...
        for key in ("icon_url", "element_icon_url")
        if m.get(key)
    }
    ...
    for m in monster_details:
        ...
        processed_list.append(
            {
                "name": m["name"],
                "icon_base64": url_to_base64_map.get(
                    m["icon_url"], TRANSPARENT_PIXEL_BASE64
                ),
                "element_icon_base64": url_to_base64_map.get(
                    m["element_icon_url"], TRANSPARENT_PIXEL_BASE64
                ),
        ...
        "groups": [
            {
                ...
                "floors": [
                    {
                        "name": "第4层",
                        "monsters": await process_monsters(
                            http_client, floor_4_data.get("Monsters")
                        ),
                    }
                ],
            }
...
async def process_area_2(http_client: httpx.AsyncClient, area_2_floors):
                ...
                "floors": [
                    {
                        "name": "第1层",
                        "monsters": await process_monsters(
                            http_client, floor_1.get("Monsters")
                        ),
                    },
                    {
                        "name": "第2层",
                        "monsters": await process_monsters(
                            http_client, floor_2.get("Monsters")
                        ),
                    },
                ],
            }
                ...
                "floors": [
                    {
                        "name": "第3层",
                        "monsters": await process_monsters(
                            http_client, floor_3.get("Monsters")
                        ),
                    },
                    {
                        "name": "第4层",
                        "monsters": await process_monsters(
                            http_client, floor_4.get("Monsters")
                        ),
                    },
                ],
            }
    ...
    if groups:
        return {
            "name": floor_1.get("AreaName", "深境之塔") if floor_1 else "深境之塔",
            "groups": groups,
        }
    return None
