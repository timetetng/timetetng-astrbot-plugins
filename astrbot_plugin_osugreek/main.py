...
def resize_greek_image(
    greek_img: Image.Image, original_width: int, original_height: int
) -> Image.Image:
    """调整字母图片大小"""
...
@register(
    "astrbot_plugin_osugreek",
    "YakumoZn",
    "在图片中央贴上神秘4k希腊字母并添加色散效果的插件",
    "1.0.1",
    "https://github.com/YakumoZn/nonebot-plugin-osugreek",
)
class OsugreekPlugin(Star):
        ...
        for params in params_list:
            try:
                ...
                if isinstance(msg_list, list):
                    for seg in msg_list:
                        if isinstance(seg, dict) and seg.get("type") in (
                            "image",
                            "mface",
                        ):
                            d = seg.get("data", {})
    ...
    async def osugreek_cmd(self, event: AstrMessageEvent):
        ...
        chromatic_intensity = (
            int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 4
        )
        ...
        if not image_url:
            for comp in event.message_obj.message:
                if isinstance(comp, Comp.Reply):
                    ...
                    if not image_url:
                        reply_id = getattr(
                            comp, "id", getattr(comp, "message_id", None)
                        )
                        if (
                            not reply_id
                            and hasattr(comp, "data")
        ...
        try:
            ...
            # 生成临时文件并发送
            temp_filename = (
                f"processed_{int(time.time() * 1000)}_{random.randint(1000, 9999)}.png"
            )
