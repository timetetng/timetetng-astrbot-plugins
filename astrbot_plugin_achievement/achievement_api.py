...
class AchievementAPI:
    ...
    async def unlock_achievement(
        ...
        user_id: str,
        achievement_id: str,
        event: AstrMessageEvent | None = None  # <--- 【核心修改点1】: 增加一个可选的 event 参数
    ) -> bool:
        """
        为用户解锁一个成就的核心逻辑。
        ...
        ach_data = self._plugin.achievement_manager.get_achievement_by_id(achievement_id)
        if not ach_data:
            logger.warning(f"尝试解锁一个不存在的成就: {achievement_id}")
            return False

        # ... (所有锁、检查、数据更新、发奖励的逻辑完全不变) ...
        ...
        async with self._plugin.unique_achievement_lock:
            if self._plugin.data_manager.has_achievement(user_id, achievement_id):
                return False
            if is_unique and self._plugin.data_manager.is_unique_achievement_claimed(achievement_id):
                return False

            self._plugin.data_manager.add_achievement_to_user(user_id, achievement_id)

            if is_unique:
                self._plugin.data_manager.claim_unique_achievement(achievement_id, user_id)

        reward_coins = ach_data.get("reward_coins", 0)
        if self._plugin.config.get("enable_rewards") and self._plugin.apis.get("economy_api") and reward_coins > 0:
        ...
        # --- 【核心修改点2】: 在解锁成功后，检查 event 并触发通知 ---
        if event:
            try:
                # 调用主插件中已经写好的通知函数
                user_name = await self._plugin._get_display_name(user_id, event.get_sender_name())
                ...
                logger.info(f"通过API调用为用户 {user_id} 发送了成就 {achievement_id} 的即时通知。")
