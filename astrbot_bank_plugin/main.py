...
class BankAPI:
    ...
    async def get_top_accounts(self, limit: int = 10) -> list[dict]:
        ...
        # 按总资产降序排序
        sorted_user_assets = sorted(user_assets, key=lambda x: x['total_asset'], reverse=True)
        
        # 返回前 limit 个结果，并修改键名为 "balance" 以兼容旧接口
        top_users = sorted_user_assets[:limit]
        return [{"user_id": user["user_id"], "balance": user["total_asset"]} for user in top_users]


@register("bank", "Gemini & YourName", "一个功能丰富的银行插件，支持定期存款和成就系统", "2.0.0", "https://github.com/AstrBotDevs/AstrBot")
class BankPlugin(Star):
    ...
    async def initialize_and_run_task(self):
        ...
        self.industry_api = await self.wait_for_api("industry_api", timeout=10) # 贷款不是核心，等待时间短点
    ...
    async def init_database(self):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            try:
                await db.execute("ALTER TABLE accounts ADD COLUMN total_interest_earned REAL NOT NULL DEFAULT 0")
    ...
    async def db_get_all_bank_users(self) -> set[str]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor_accounts = await db.execute("SELECT user_id FROM accounts WHERE balance > 0")
    ...
    async def db_get_balance(self, user_id: str) -> float:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("SELECT balance FROM accounts WHERE user_id = ?", (user_id,))
            row = await cursor.fetchone()
            return round(row[0], 2) if row else 0.0

    async def db_get_account_info(self, user_id: str) -> dict:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("SELECT balance, total_interest_earned FROM accounts WHERE user_id = ?", (user_id,))
            row = await cursor.fetchone()
            if row:
                return {"balance": round(row[0], 2), "total_interest_earned": round(row[1], 2)}
    ...
    async def db_update_balance(self, user_id: str, amount_change: float) -> float:
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute("UPDATE accounts SET balance = balance + ? WHERE user_id = ?", (amount_change, user_id))
            await db.commit()
            return await self.db_get_balance(user_id)

    async def db_get_loan(self, user_id: str) -> dict | None:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("SELECT principal, amount_due, interest_rate, loan_date FROM loans WHERE user_id = ?", (user_id,))
            row = await cursor.fetchone()
            if row:
                return {"principal": row[0], "amount_due": round(row[1], 2), "interest_rate": row[2], "loan_date": row[3]}
            return None

    async def db_add_fixed_deposit(self, user_id: str, amount: float, weeks: int) -> str:
        deposit_id = os.urandom(4).hex()
        start_date = datetime.now()
        end_date = start_date + timedelta(weeks=weeks)
        interest_rate = self.config.savings_interest_rate * self.config.fixed_deposit_interest_multiplier
        
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                "INSERT INTO fixed_deposits (deposit_id, user_id, principal, interest_rate, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?)",
                (deposit_id, user_id, amount, interest_rate, start_date.isoformat(), end_date.isoformat())
            )
            await db.commit()
        return deposit_id
    ...
    async def db_get_fixed_deposit(self, deposit_id: str) -> dict | None:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("SELECT * FROM fixed_deposits WHERE deposit_id = ?", (deposit_id,))
            row = await cursor.fetchone()
            if not row: return None
            return {
                "deposit_id": row[0], "user_id": row[1], "principal": row[2], 
                "interest_rate": row[3], "start_date": row[4], "end_date": row[5]
            }

    async def db_get_all_fixed_deposits(self, user_id: str) -> list[dict]:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("SELECT deposit_id, principal, end_date FROM fixed_deposits WHERE user_id = ? ORDER BY end_date", (user_id,))
            rows = await cursor.fetchall()
            return [{"deposit_id": r[0], "principal": r[1], "end_date": r[2]} for r in rows]

    async def db_delete_fixed_deposit(self, deposit_id: str):
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("DELETE FROM fixed_deposits WHERE deposit_id = ?", (deposit_id,))
            await db.commit()
    ...
    async def bank_help(self, event: AstrMessageEvent):
        yield event.plain_result(
            ...
            "\n"
            "【定期存款】(利息更高!)\n"
            f"/定期存款 [金额] [周数] - 存入一笔定期，到期前无法取出。\n"
            f"/查询定期 - 查看你所有的定期存款。\n"
            f"/取出定期 [存款ID] - 取出已到期的定期存款本息。\n"
            "\n"
            "【贷款服务】\n"
            "/贷款信息 - 查看当前贷款详情或规则。\n"
            "/贷款 [金额] - 抵押资产申请贷款。\n"
    ...
    async def deposit(self, event: AstrMessageEvent, amount: int):
        ...
        if current_coins < amount:
            yield event.plain_result(f"您的现金不足！当前现金: {current_coins:,.2f} 金币。")
            return
            
        is_first_deposit = (await self.db_get_balance(user_id)) == 0 and (len(await self.db_get_all_fixed_deposits(user_id)) == 0)
        ...
        if success:
            ...
            if self.achievement_api and is_first_deposit:
                await self.achievement_api.unlock_achievement(user_id, "bank_first_deposit", event=event)
    ...
    async def withdraw_fixed_deposit(self, event: AstrMessageEvent, deposit_id: str):
        ...
        if datetime.now() < end_date:
            yield event.plain_result(f"该笔存款尚未到期（到期日: {end_date.strftime('%Y-%m-%d')}），无法取出。")
            return
        ...
        await self.economy_api.add_coins(user_id, final_amount, f"取出定期存款{deposit_id}")

        if self.achievement_api and (end_date - start_date) >= timedelta(days=6.9):
            await self.achievement_api.unlock_achievement(user_id, "bank_fixed_deposit_success", event=event)
    ...
    async def loan_info(self, event: AstrMessageEvent):
        ...
        if not self.industry_api:
            yield event.plain_result("抱歉，由于未安装虚拟产业插件，本银行暂不提供贷款服务。")
            return
    ...
    async def apply_loan(self, event: AstrMessageEvent, amount: int):
        ...
        if amount > max_loan_amount:
            yield event.plain_result(f"您的资产最多只能贷款 {int(max_loan_amount)} 金币，无法申请 {amount} 金币。")
            return
            
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                "INSERT INTO loans (user_id, principal, amount_due, interest_rate, loan_date) VALUES (?, ?, ?, ?, ?)",
                (user_id, amount, amount, self.config.loan_interest_rate, datetime.now().strftime("%Y-%m-%d"))
            )
            await db.commit()
    ...
    async def repay_loan(self, event: AstrMessageEvent, amount: int):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            if new_amount_due <= 0.01:
                ...
                yield event.plain_result(f"🎉 恭喜您！您已成功还清所有贷款！")
            else:
                await db.execute("UPDATE loans SET amount_due = ? WHERE user_id = ?", (new_amount_due, user_id))
                yield event.plain_result(f"✅ 还款成功！\n本次还款: {repay_amount} 金币\n剩余应还: {round(new_amount_due, 2)} 金币。")
            await db.commit()
    ...
    async def repay_all_loan(self, event: AstrMessageEvent):
        ...
        if current_coins < amount_to_repay:
            yield event.plain_result(f"您的现金不足以还清全部贷款！\n需要: {amount_to_repay} 金币\n持有: {current_coins} 金币")
            return

        success = await self.economy_api.add_coins(user_id, -amount_to_repay, "还清银行贷款")
        if not success:
            yield event.plain_result("还款失败，现金扣除时发生错误。")
            return
        ...
        yield event.plain_result(f"🎉 恭喜您！您已成功使用 {amount_to_repay} 金币还清所有贷款！")
    ...
    async def interest_calculation_task(self):
        ...
        while True:
            now = datetime.now()
            target_time = now.replace(hour=self.config.interest_calculation_hour, minute=0, second=0, microsecond=0)
            if now > target_time:
                target_time += timedelta(days=1)
            
            sleep_seconds = (target_time - now).total_seconds()
            logger.info(f"银行插件：下一次利息结算在 {target_time}, 等待 {sleep_seconds:.0f} 秒。")
            await asyncio.sleep(sleep_seconds)
            
            logger.info(f"银行插件：开始执行每日利息结算...")
            async with aiosqlite.connect(self.db_path) as db:
                # 结算活期利息
                ...
                await db.execute("UPDATE loans SET amount_due = amount_due * (1 + ?)", (loan_rate,))
                ...
                if self.achievement_api:
                    ...
                    for user_id, loan_date_str in overdue_loans:
                        loan_date = datetime.fromisoformat(loan_date_str.split(" ")[0]) # 兼容旧格式
                        if (datetime.now() - loan_date) > timedelta(days=3):
                            # 使用静默解锁，避免半夜打扰用户
                            await self.achievement_api.unlock_achievement(user_id, "bank_loan_overdue_3_days", event=event)
                            logger.info(f"用户 {user_id} 贷款逾期超过3天，尝试静默触发成就。")
