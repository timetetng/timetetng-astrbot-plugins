...
try:
    ...
    logger.warning(
        f"无法从本地导入视频解析模块 (bili_get.py, file_send_server.py)，视频推送功能将受限: {e}"
    )
...
class BiliAtNotifierPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.accounts_config_internal: list[dict] = []
        self.api_clients: dict[str, BiliApiClient] = {}
        self.last_at_ids: dict[str, int] = {}
        ...
        self.max_video_size = (
            self.config.get("max_video_size", 100) * 1024 * 1024
        )  # 转为字节
        ...
        for i in range(num_accounts):
            account_cfg = {
                ...
                if i < len(user_agents) and user_agents[i]
                else self.global_user_agent,
            }
            # 基础验证
            if not account_cfg["SESSDATA"] or not account_cfg["bili_jct"]:
                logger.warning(
                    f"跳过索引 {i} 的账号配置，因为 SESSDATA 或 bili_jct 为空。"
                )
                continue
            if not account_cfg["user_agent"]:
                logger.warning(
                    f"跳过索引 {i} (标签: {account_cfg['account_label']}) 的账号配置，因为 User-Agent 为空（请检查账号配置或全局配置）。"
                )
                continue
        ...
        for account_cfg in self.accounts_config_internal:
            ...
            try:
                ...
                logger.info(
                    f"成功初始化 Bilibili 账号: '{account_label}' (Key: {account_key})"
                )
                valid_accounts += 1
                ...
                logger.error(
                    f"初始化账号 '{account_label}' 时发生未知错误: {e}", exc_info=True
                )
        ...
        try:
            ...
            logger.info(
                f"Bilibili @ 消息监听插件已启动 ({valid_accounts}个账号)，开始轮询..."
            )
        except Exception as e:
    ...
    async def poll_at_mentions(self):
        ...
        while True:
            ...
            for account_key in account_keys:
                ...
                if not api_client:
                    logger.warning(
                        f"账号 '{account_label}' (Key: {account_key}) 的客户端实例丢失，跳过本次检查。"
                    )
                    continue
                ...
                try:
                    while has_more:
                        at_data = await api_client.get_at_mentions(
                            cursor_id=next_cursor_id, cursor_time=next_cursor_time
                        )

                        if at_data is None:
                            logger.warning(
                                f"账号 '{account_label}': 获取 @ 消息失败 (API返回None)，跳过此账号本轮检查。"
                            )
                            has_more = False
                            break  # 停止此账号的本轮检查
                        ...
                        if not items:
                            ...
                            current_max_id_this_poll = max(
                                current_max_id_this_poll, page_max_id
                            )

                            if is_first:
                                self.last_at_ids[account_key] = (
                                    current_max_id_this_poll  # 首次运行，记录最新 ID
                                )
                                logger.info(
                                    f"账号 '{account_label}': 首次运行，记录最新消息 ID: {self.last_at_ids[account_key]} (来自第 {page_count} 页)，本次不推送。"
                                )
                                ...
                                if should_stop_paging or is_end:
                                    ...
                                    page_count += 1
                                    if (
                                        next_cursor_id is None
                                        or next_cursor_time is None
                                    ):
                                        logger.warning(
                                            f"账号 '{account_label}': 分页 cursor 信息不完整，停止分页。Cursor: {cursor}"
                                        )
                                        has_more = False
                    ...
                    if new_messages:
                        logger.info(
                            f"账号 '{account_label}': 共发现 {len(new_messages)} 条新 @ 消息，准备推送..."
                        )
                        await self.process_and_send_messages(
                            new_messages, account_label
                        )  # 传入账号标签

                        self.last_at_ids[account_key] = max(
                            last_id, current_max_id_this_poll
                        )  # 更新最新处理的消息ID
                        logger.info(
                            f"账号 '{account_label}': 推送完成，更新 last_at_id 为 {self.last_at_ids[account_key]}"
                        )

                    elif not is_first:
                        if current_max_id_this_poll > last_id:
                            logger.info(
                                f"账号 '{account_label}': 检查到本轮最大ID ({current_max_id_this_poll}) 大于旧ID ({last_id})，但无消息推送（可能全部被过滤）。更新 last_at_id。"
                            )
                    ...
                    logger.error(
                        f"检查账号 '{account_label}' 时发生错误: {e}", exc_info=True
                    )
            ...
            # 所有账号检查完毕后，等待下一个轮询周期
            logger.info(
                f"所有账号检查完毕，将在 {self.polling_interval} 秒后进行下一轮检查。"
            )
            await asyncio.sleep(self.polling_interval)

    async def process_and_send_messages(
        self, messages: list, account_label: str
    ):  # 增加 account_label 参数
        ...
        for msg_data in messages:
            try:
                ...
                at_time_str = datetime.fromtimestamp(at_time_ts).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )

                if not video_url or not video_url.startswith("http"):
                    logger.warning(
                        f"账号 '{account_label}': 消息 ID {msg_data.get('id')} 不包含有效的视频链接，跳过。"
                    )
                    continue

                logger.info(
                    f"账号 '{account_label}': 正在处理来自 {sender_name} 的 @ 消息 (ID: {msg_data.get('id')}), 视频链接: {video_url}"
                )

                # 1. 构造并发送文本通知消息 (加入账号标签)
                notification_text = (
                    f"📢 账号「{account_label}」收到一坨屎！\n👤 谁拉的: {sender_name}"
                )
                ...
                for target_umo in self.target_umos:
                    try:
                        logger.info(
                            f"账号 '{account_label}': 准备推送 @ 文本通知 (ID: {msg_data.get('id')}) 到 {target_umo}"
                        )
                        await self.context.send_message(target_umo, notification_chain)
                        logger.info(
                            f"账号 '{account_label}': 成功推送 @ 文本通知 (ID: {msg_data.get('id')}) 到 {target_umo}"
                        )
                        await asyncio.sleep(0.5)
                    except Exception as send_e:
                        logger.error(
                            f"推送 @ 文本通知到 {target_umo} 失败: {send_e}",
                            exc_info=True,
                        )
                ...
                if VIDEO_ANALYSIS_ENABLED and process_bili_video:
                    logger.info(
                        f"账号 '{account_label}': 开始使用 video_analysis 插件解析: {video_url}"
                    )
                    ...
                    if analysis_result and analysis_result.get("video_path"):
                        ...
                        logger.info(
                            f"账号 '{account_label}': 视频解析成功: {video_title}, 路径: {video_path}"
                        )

                        nap_file_path = video_path
                        nap_server_address = self.config.get(
                        ...
                        if (
                            ...
                            try:
                                logger.info(
                                    f"尝试通过NAP发送文件: {video_path} 到 {nap_server_address}:{nap_server_port}"
                                )
                                nap_file_path = await nap_send_file(
                                    video_path,
                                    HOST=nap_server_address,
                                    PORT=nap_server_port,
                                )
                                logger.info(f"NAP文件路径: {nap_file_path}")
                            except Exception as nap_e:
                                logger.error(
                                    f"通过NAP发送文件失败: {nap_e}, 将尝试使用本地路径。"
                                )
                                nap_file_path = video_path
                        elif nap_server_address != "localhost":
                            logger.warning(
                                "配置了NAP服务器地址但端口未配置或发送函数未导入，使用本地路径。"
                            )
                        ...
                        try:
                            ...
                            if file_size > self.max_video_size:
                                logger.warning(
                                    f"视频文件过大 ({file_size / 1024 / 1024:.2f} MB > {self.max_video_size / 1024 / 1024:.2f} MB)，尝试作为文件发送。"
                                )
                                media_component = File(
                                    file=nap_file_path,
                                    name=os.path.basename(video_path),
                                )
                            else:
                                media_component = Video.fromFileSystem(
                                    path=nap_file_path
                                )
                            ...
                            logger.error(
                                f"视频文件未找到: {video_path} (用于检查大小) 或 {nap_file_path} (用于创建组件)，无法发送视频。"
                            )
                            media_component = Plain(
                                f"❌ 视频文件丢失: {video_title}\n🔗 原始链接: {video_url}"
                            )
                        except Exception as comp_e:
                            logger.error(f"创建媒体组件时出错: {comp_e}", exc_info=True)
                            media_component = Plain(
                                f"❌ 处理视频失败: {video_title}\n🔗 原始链接: {video_url}"
                            )

                    else:
                        logger.warning(
                            f"账号 '{account_label}': 视频解析失败或未找到 video_path: {video_url}"
                        )
                        media_component = Plain(
                            f"⚠️ 视频解析失败，请手动查看: {video_url}"
                        )

                else:
                    if not VIDEO_ANALYSIS_ENABLED:
                        logger.warning(
                            f"账号 '{account_label}': 视频解析功能未启用 (VIDEO_ANALYSIS_ENABLED=False)，发送链接。"
                        )
                    else:
                        logger.warning(
                            f"账号 '{account_label}': process_bili_video 函数未导入 (bili_get.py 可能缺失或导入失败)，发送链接。"
                        )
                    media_component = Plain(
                        f"🔗 相关视频 (解析功能未启用): {video_url}"
                    )
                ...
                if media_component:
                    ...
                    for target_umo in self.target_umos:
                        try:
                            logger.info(
                                f"账号 '{account_label}': 准备推送视频/文件 (来自 @ 消息 ID: {msg_data.get('id')}) 到 {target_umo}"
                            )
                            await self.context.send_message(
                                target_umo, message_chain_to_send
                            )
                            logger.info(
                                f"账号 '{account_label}': 成功推送视频/文件 (来自 @ 消息 ID: {msg_data.get('id')}) 到 {target_umo}"
                            )
                            await asyncio.sleep(self.send_delay)
                        except Exception as send_e:
                            logger.error(
                                f"推送视频/文件到 {target_umo} 失败: {send_e}",
                                exc_info=True,
                            )
                else:
                    logger.warning(
                        f"账号 '{account_label}': 没有可发送的媒体组件 (来自 @ 消息 ID: {msg_data.get('id')})，跳过发送。"
                    )

            except Exception as outer_e:
                logger.error(
                    f"账号 '{account_label}': 处理 @ 消息 (ID: {msg_data.get('id')}) 时发生外部错误: {outer_e}",
                    exc_info=True,
                )
