...
class BiliApiClient:
    ...
    async def _safe_json_from_response(
        ...
        try:
            ...
            logger.error(
                f"解析 B站 API JSON 失败: status={response.status}, url='{response.url}', 片段='{preview}', 错误: {e}"
            )
            return None

    async def get_at_mentions(
        ...
        if cursor_id is not None and cursor_time is not None:
            ...
            logger.debug(
                f"请求下一页 @ 消息: cursor_id={cursor_id}, cursor_time={cursor_time}"
            )
        else:
            ...
            async with session.get(AT_FEED_URL, params=params) as response:
                if response.status == 200:
                    ...
                    if isinstance(data, dict) and data.get("code") == 0:
                        logger.debug(
                            f"成功获取 @ 消息，数量: {len(data.get('data', {}).get('items', []))}"
                        )
                        return data.get("data")
                    elif isinstance(data, dict) and data.get("code") == -101:
                        logger.error(
                            f"获取 Bilibili @ 消息失败: Cookie 失效或未登录 ({data})"
                        )
                    ...
                    logger.error(
                        f"获取 Bilibili @ 消息 HTTP 错误: {response.status}, message='{response.reason}', url='{response.url}', body='{body_preview[:200]}'"
                    )
                    return None
