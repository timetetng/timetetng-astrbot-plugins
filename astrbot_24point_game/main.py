...
try:
    ...
    raise ImportError(
        "缺少 'asteval' 库，请运行 'pip install asteval' 或在插件的 requirements.txt 中添加它。"
    )
...
class GameState:
    ...
    def __init__(
        ...
        self.participants: Dict[
            str, Dict[str, Any]
        ] = {}  # {user_id: {"name": "xxx", "score": 100, "expr": "..."}}
...
class Game24Plugin(Star):
    ...
    async def _save_solution_leaderboard(self):
        try:
            ...
            # 排序并只保留前10名
            self.solution_leaderboard.sort(
    ...
    def _setup_safe_eval(self) -> Interpreter:
        ...
        for func in ["open", "eval", "exec", "import_module", "__import__"]:
            if func in aeval.symtable:
                del aeval.symtable[func]
    ...
    def _find_all_solutions(self, nums: List[float]) -> Dict[float, Set[str]]:
        if len(nums) == 1:
            return {
                nums[0]: {
                    str(int(nums[0])) if nums[0] == int(nums[0]) else str(nums[0])
                }
            }
        results = {}
    ...
    def _generate_problem(
        self, difficulty: str = "普通"
    ) -> Optional[Tuple[List[int], List[str], int]]:
        num_range = {"简单": (1, 7), "普通": (1, 10), "困难": (1, 13)}.get(
            difficulty, (1, 10)
        )
        ...
        for _ in range(500):  # 增加尝试次数以找到合适的题目
            ...
            try:
                ...
                if verified_solutions:  # 使用严格验证后的解列表
                    ...
                    # 根据难度调整筛选条件
                    if difficulty == "困难" and num_solutions > 15:
                        continue
                    if difficulty == "简单" and num_solutions < 5:
                        continue
    ...
    def _calculate_reward(
        self, state: GameState, processed_expression: str
    ) -> Tuple[int, str, float]:
        """计算传统计时模式的奖励"""
        ...
        if "**" in processed_expression:
            solution_bonus += 25
        if "factorial" in processed_expression:
            solution_bonus += 40
        total_reward = base_reward + difficulty_bonus + speed_bonus + solution_bonus
        details = (
            f"基础分({base_reward}) + 难度分({difficulty_bonus}) + "
    ...
    async def start_game_command(self, event: AstrMessageEvent):
        ...
        if session_id in self.active_games:
            game_mode_text = (
                "比分赛" if self.active_games[session_id].mode == "score" else "挑战赛"
            )
        ...
        difficulty = "普通"
        if "简单" in difficulty_text:
            difficulty = "简单"
        elif "困难" in difficulty_text:
            difficulty = "困难"
        ...
        timeout_task = asyncio.create_task(
            self._game_timeout(session_id, event, self.TIMED_MODE_TIMEOUT, "timed")
        )
        self.active_games[session_id] = GameState(
            numbers, solutions, diff_score, timeout_task, "timed"
        )
    ...
    async def start_score_game_command(self, event: AstrMessageEvent):
        ...
        numbers, solutions, diff_score = problem
        timeout_task = asyncio.create_task(
            self._game_timeout(session_id, event, self.SCORE_MODE_TIMEOUT, "score")
        )
        self.active_games[session_id] = GameState(
            numbers, solutions, diff_score, timeout_task, "score"
        )
    ...
    async def answer_hook(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        is_correct, message, processed_expr = self._check_user_expression(
            user_answer, state.numbers
        )
        if not is_correct:
            # 只有计时模式下才提示错误答案
            if state.mode == "timed":
                await event.send(
                    event.plain_result(f"🤔 @{event.get_sender_name()}，{message}")
                )
            event.stop_event()
            return
        ...
        if state.mode == "timed":
            ...
            await self._handle_score_mode_submit(
                event, state, user_answer, processed_expr
            )
    ...
    async def end_game_command(self, event: AstrMessageEvent):
        session_id = event.get_group_id() or event.get_sender_id()
        if (
            session_id in self.active_games
            and self.active_games[session_id].mode == "timed"
        ):
    ...
    async def end_score_game_command(self, event: AstrMessageEvent):
        ...
        if (
            session_id in self.active_games
            and self.active_games[session_id].mode == "score"
        ):
            state = self.active_games.pop(session_id)
            if state.is_active:
                state.is_active = False
                state.timeout_task.cancel()
                await self._finalize_score_game(
                    state,
                    event.unified_msg_origin,
                    f"比赛已由 @{event.get_sender_name()} 提前结束！",
                )
        else:
    ...
    async def show_leaderboard(self, event: AstrMessageEvent):
        ...
        sorted_stats = sorted(
            stats_list, key=lambda x: x.get("total_score", 0), reverse=True
        )
    ...
    async def show_solution_leaderboard(self, event: AstrMessageEvent):
        ...
        for i, entry in enumerate(self.solution_leaderboard[:10]):
            ...
            line = (
                f"👑 Top {rank}: {entry.get('score')} 分 - By {entry.get('user_name', '匿名宗师')}\n"
                f"   题目: [{nums_str}]\n"
    ...
    async def _game_timeout(
        self, session_id: str, event: AstrMessageEvent, timeout: float, mode: str
    ):
        try:
            await asyncio.sleep(timeout)
            if (
                session_id in self.active_games
                and self.active_games[session_id].is_active
            ):
                state = self.active_games.pop(session_id)
                state.is_active = False
                ...
                if mode == "timed":
                    ...
                    await self._finalize_score_game(
                        state, event.unified_msg_origin, "⏱️ 时间到！比赛结束"
                    )
    ...
    def _check_user_expression(
        self, expression: str, numbers: List[int]
        ...
        try:
            ...
            logger.error(
                f"表达式计算失败: {expression} -> {processed_expr} | 错误: {e}"
            )
            return False, "你的表达式好像有点问题，我算不出来呢。", None

    async def _handle_timed_mode_win(
        self,
        ...
        total_reward, details, time_taken = self._calculate_reward(
        ...
        reward_msg = ""
        if self.economy_api:
            awarded_amount, reward_msg_part = await self._award_coins(
                user_id, total_reward, "24点计时赛胜利"
            )
    ...
    async def _handle_score_mode_submit(
        self,
        event: AstrMessageEvent,
        state: GameState,
        user_answer: str,
        ...
        for participant_id, p_data in state.participants.items():
            ...
            if p_data["processed_expr"] == processed_expr:
                ...
                await event.send(
                    event.plain_result(
                        f"@{user_name}，这个解法已经被 @{submitter_name} 提交过了哦，换个思路吧！"
                    )
                )
                return
        ...
        if current_best and score <= current_best["score"]:
            await event.send(
                event.plain_result(
                    f"@{user_name}，这个解法得分({score})没有你之前的解法({current_best['score']})高哦，再想想有没有更秀的！"
                )
            )
            return
    ...
    async def _finalize_score_game(self, state: GameState, origin: Dict, title: str):
        ...
        if not state.participants:
            ...
            await self.context.send_message(
                origin, MessageChain().message(timeout_message)
            )
            return  # 结束函数
        ...
        sorted_participants = sorted(
            state.participants.items(), key=lambda item: item[1]["score"], reverse=True
        )
        ...
        if self.economy_api and total_score > 0:
            for user_id, p_data in sorted_participants:
                potential_reward = math.ceil(
                    self.SCORE_MODE_PRIZE_POOL * (p_data["score"] / total_score)
                )
                awarded_amount, reward_msg_part = await self._award_coins(
                    user_id, potential_reward, "24点比分赛奖励"
                )

                awarded_coins_info.append(
                    (p_data["name"], p_data["score"], awarded_amount)
                )
        ...
        if awarded_coins_info:
            ...
            for i, (user_id, p_data) in enumerate(sorted_participants):
                result_lines.append(
                    f"第 {i + 1} 名: @{p_data['name']} ({p_data['score']}分)"
                )
